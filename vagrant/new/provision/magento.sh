...
#    sudo -i -u "vagrant" composer create-project --no-install --no-progress --repository=https://repo.magento.com/ magento/project-"$VM_MAGENTO_EDITION"-edition="$VM_MAGENTO_VERSION" "magento" -d "/vagrant"
#fi
# Install sample data
...
# Composer install
#sudo -i -u "vagrant" composer install -d "/vagrant/magento" --no-progress --no-suggest
